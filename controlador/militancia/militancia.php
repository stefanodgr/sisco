<?php
	...
	if($ctrlAcceso){
        ...
        switch($case){
            case 0:
                $vista = $rutaDir.'vista/militancia/militancia.tpl';
            ...
            case 1:	
                ...
                if($estado == 'inicial'){
                    unset($estado);
                    $titulo = 'Datos Generales:Militancia';
                    $rutaReferencial = 'controlador/militancia/militancia.php?case='.$case;
                    $vista = $rutaDir.'vista/militancia/militancia.tpl';
                    $TBS->LoadTemplate($vista);
                }
                else{
                    $descTabla 		= 'Listado de Militancia';
                    $nombreTabla	= "militancia";
                    $idTabla 		= "militancia_id";	
                    $siglas 		= 'lmili';
                    $tipoCampoPk	= 'serial';	
                    $encabezado		= array('Nombre');
                    $columna 		= array('militancia_desc');
                    $tipo 			= array('text');
				...
				// $select = array($select_extintor_status);
                    ...
                    $rutaJson 	= $rutaDir.'json/militancia.php?case=0';
