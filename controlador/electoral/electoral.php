<?php
	...
	if($ctrlAcceso){
        ...
        switch($case){
            case 0:
                $vista = $rutaDir.'vista/electoral/electoral.tpl';
            ...
            case 1:	
                ...
                if($estado == 'inicial'){
                    unset($estado);
                    $titulo = 'Datos Generales:Electoral';
                    $rutaReferencial = 'controlador/electoral/electoral.php?case='.$case;
                    $vista = $rutaDir.'vista/electoral/electoral.tpl';
                    $TBS->LoadTemplate($vista);
                }
                else{
                    $descTabla 		= 'Listado de Electoral';
                    $nombreTabla	= "electoral";
                    $idTabla 		= "electoral_id";	
                    $siglas 		= 'lelec';
                    $tipoCampoPk	= 'serial';	
                    $encabezado		= array('Nombre');
                    $columna 		= array('electoral_desc');
                    $tipo 			= array('text');
				...
				// $select = array($select_extintor_status);
                    ...
                    $rutaJson 	= $rutaDir.'json/electoral.php?case=0';
